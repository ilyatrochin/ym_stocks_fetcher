...
3. Скопировать **Идентификатор кампании** (числовой) — это `YM_CAMPAIGN_ID`.
4. Там же скопировать **ID бизнес-аккаунта** — это `YM_BUSINESS_ID`
   (другой идентификатор, не путать).
5. Вкладка **API-ключи** → **Создать новый ключ**:
   - Имя: `openclaw-ym-fetcher`
   - Доступы:
     - ✅ **Остатки и оборачиваемость** (модуль 1)
     - ✅ **Информация о складах** (модуль 1)
     - ✅ **Заказы** (модуль 2)
     - ✅ **Получение информации по FBY-заявкам** (`supplies-management:read-only`) (модуль 3)
   - Срок: максимальный
6. Скопировать ключ сразу (показывается один раз).
...
Для stocks должны увидеть в выводе:
- `Pagination complete after N pages`
- `DRY RUN: would write N raw + N snapshot rows`
- Несколько примеров строк

Для supplies должны увидеть:
- `Active request #ID (...): status=..., eta=..., wh=...` — для каждой активной заявки
- `DRY RUN: N planned rows`

Если ошибка `401 Unauthorized` — неверный API-key или campaignId. Если
`403` — у ключа нет нужного доступа. Для `supplies` 403 значит, что
нет права `supplies-management:read-only` — вернись к шагу 2 и
отредактируй ключ.
