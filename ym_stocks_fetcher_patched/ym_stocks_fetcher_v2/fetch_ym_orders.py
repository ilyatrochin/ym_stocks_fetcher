...
class YMOrdersClient:
    ...
    def fetch_orders(self, from_date: date, to_date: date) -> Iterator[dict]:
        ...
        while True:
            ...
            # API ожидает даты в формате DD-MM-YYYY
            body = {
                "dates": {
...
def parse_order(order: dict, fetch_ts: str, request_id: str) -> list[OrderItemRow]:
    ...
    date_created = _parse_date(order.get("creationDate") or order.get("createdAt"))
    date_delivered = ""
    if status == DELIVERED_STATUS:
        date_delivered = (
            _parse_date(order.get("statusUpdateDate"))
            or _parse_date((order.get("delivery") or {}).get("deliveryDate"))
        )

    fake = "Да" if order.get("fake") else "Нет"

    delivery = order.get("delivery") or {}
    region_obj = delivery.get("region") or {}
    region = region_obj.get("name", "") if isinstance(region_obj, dict) else ""

    warehouse_id: int | str = ""
    if "warehouseId" in delivery:
        warehouse_id = delivery["warehouseId"]
    elif isinstance(delivery.get("warehouse"), dict):
        warehouse_id = delivery["warehouse"].get("id", "")
    ...
    for item in order.get("items", []):
        ...
        subsidy = sum(float(s.get("amount", 0) or 0)
                      for s in (item.get("subsidies") or []))
...
# Main
# ----------------------------------------------------------------------
def run(from_date: date, to_date: date, dry_run: bool = False) -> FetchResult:
    started = time.time()
    ...
    try:
        ...
        client = YMOrdersClient(api_key, business_id)
